...
contract CrowdFunding {
    ...
    function createCompaign(address _owner, string memory _title, string memory _discription, uint256 _target, uint256 _deadline, string memory _image) public returns (uint256) {
        Compaign storage compaign = compaigns[numberOfCompaigns];
        // is everything okay?
        require(compaign.deadline < block.timestamp, "The deadline a date in the future.");
        compaign.owner = _owner;
        compaign.title = _title;
        compaign.discription = _discription;
